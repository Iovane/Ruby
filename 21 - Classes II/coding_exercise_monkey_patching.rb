...
class Hash
    def common_keys_and_values
        # into_array = self.to_a.flatten
        # repeated = []
        #
        # into_array.each { |element| repeated.push(element) if into_array.count(element) > 1 && !repeated.include?(element) }
        #
        # repeated
