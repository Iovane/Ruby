...
def double_elements(numbers)
    ...
    numbers.each { |number| doubled.push(number * 2) }
...
def extract_long_words(words)
    only_long = []
    words.each { |word| only_long.push(word) if word.length > 7}
...
def pastas_and_sauces(pastas, sauces)
    ...
    pastas.each do |pasta|
        sauces.each do |sauce|
            combined.push("#{pasta.capitalize} with #{sauce.capitalize} sauce")
        end
    end
